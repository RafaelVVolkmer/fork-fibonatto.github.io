...
void add_code_block(struct str_view lang, struct str_view code) {
	...
	buf_append_attr_escaped(&g_html_buf,
				code_block_pre,
				strlen(code_block_pre));
...
void add_image(const char *path,
	       ...
	       int is_lcp) {
	buf_append(&g_html_buf,
		   "<p class=\"para\"><span class=\"img-placeholder\" "
		   "data-src=\"");
	...
	if (is_lcp)
		buf_append(&g_html_buf, " data-lcp=\"1\"");
	if (width > 0)
		buf_printf(&g_html_buf, " data-width=\"%d\"", width);
	if (height > 0)
		buf_printf(&g_html_buf, " data-height=\"%d\"", height);
...
void add_blog_entry(const char *title, const char *date, const char *slug) {
	...
	buf_append(&g_html_buf,
		   "<span "
		   "style=\"color:var(--dim-text-color);min-width:100px;\">");
...
void add_bar(int h,
	...
	buf_printf(&g_html_buf,
		   "<div style=\"width:%dpx;height:%dpx;border:1px solid "
		   "var(--text-color);display:flex;flex-direction:column;\">",
		   w,
		   h);
	for (int i = 0; i < n; i++) {
		...
		buf_printf(&g_html_buf,
			   ...
			   (double) (pcts[i] * 100.0f),
			   (double) opacities[i]);
		if (styles[i] == BAR_SEG_HATCHED) {
			buf_append(&g_html_buf,
				   "background-image:repeating-linear-gradient("
				   "45deg,transparent,transparent 2px,var(");
...
void ui_end_render(void) {
	static const char overflow_html[] = "<p class=\"para\">Render buffer "
					    "exceeded capacity.</p>";
	sys_set_html("#feed",
		     buf_overflowed(&g_html_buf) ? overflow_html
						 : g_html_buf.data);
}
...
void ui_render_header(void) {
	...
	add_nav_link(header.home_label,
		     header.item_style,
		     header.home_id,
		     "nav-right-group",
		     0);
	add_nav_link(header.blog_label,
		     header.item_style,
		     header.blog_id,
		     "nav-right-group",
		     1);
	add_theme_toggle(theme_label,
			 header.item_style,
			 header.theme_id,
			 "nav-right-group");
}
...
EM_JS(void,
      add_nav_group,
      (const char *id, const char *container_id, const char *style),
      {
	      const parent
		  = document.getElementById(UTF8ToString(container_id));
	      if (!parent)
		      return;
...
void update_seo_metadata(const char *t, const char *d, const char *u) {
	sys_set_meta(t, d, u);
}
