...
static void skip_ws(MathParser *p) {
	while (p->cur < p->end && is_space_uc((unsigned char) *p->cur))
		p->cur++;
}
...
static bool match_cmd(MathParser *p, const char *s) {
	...
	if (p->cur + len <= p->end && strncmp(p->cur, s, len) == 0) {
		if (is_alpha_uc((unsigned char) *s)) {
			if (p->cur + len < p->end
			    && is_alpha_uc((unsigned char) p->cur[len]))
				return false;
		}
		p->cur += len;
		return true;
...
static void emit_styled_atom(MathParser *p, const char *mathvariant) {
	...
	if (strcmp(mathvariant, "normal") == 0 && text_len > 1
	    && is_plain_identifier(text_start, text_start + text_len)) {
...
static bool parse_atom_span(MathParser *p, AtomSpan *atom) {
	...
	if (is_digit_uc((unsigned char) c)) {
		while (p->cur < p->end
		       && (is_digit_uc((unsigned char) *p->cur)
			   || *p->cur == '.'))
			p->cur++;
	...
	switch (atom->kind) {
		case ATOM_GROUP :
			if (atom->end > atom->start + 1) {
				...
				if (inner_end > atom->start
				    && *(inner_end - 1) == '}')
					inner_end--;
				MathParser inner = {.cur = atom->start + 1,
						    .end = inner_end,
						    .b	 = b};
				buf_append(b, "<mrow>");
				parse_expr(&inner);
			...
			break;
		case ATOM_COMMAND : {
			MathParser nested = {.cur = atom->start + 1,
					     .end = atom->end,
					     .b	  = b};
...
static void emit_scripts(Buffer *b,
			 const AtomSpan *base,
			 const AtomSpan *sub,
			 bool has_sub,
			 const AtomSpan *sup,
